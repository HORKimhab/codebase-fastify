...
export default fp(async function (fastify: FastifyInstance) {
  ...
  if (!swaggerUser || !swaggerPassword) {
    fastify.log.warn('Swagger UI is locked because SWAGGER_USER or SWAGGER_PASSWORD is missing.');
  }

  function unauthorized(reply: any) {
    reply.code(401).header('WWW-Authenticate', 'Basic realm="Swagger Docs"').send({ error: 'Unauthorized' });
  }
